...
<div class="global-search">
    ...
    @if($term !== '')
        ...
        <div class="result-sections">
            ...
            @if($externalInvoices->isNotEmpty())<section class="result-section"><h3>{{ __('messages.external_invoices') }} <span>{{ $externalInvoices->count() }}</span></h3><div class="result-list">@foreach($externalInvoices as $invoice)<a href="{{ $invoice->customer_id ? '/customers/'.$invoice->customer_id : '#' }}" class="result-item"><div class="result-main"><div class="result-title">{{ $invoice->invoice_no }}</div><div class="result-meta">{{ $invoice->customer?->name ?: __('messages.unlinked') }} · {{ $invoice->invoice_date?->format('Y/m/d') }}</div></div><div class="result-amount in">{{ number_format($invoice->amount,2) }} {{ $companyCurrency }}</div></a>@endforeach</div></section>@endif
        </div>
    @else
