...
<form class="da-panel" method="post" action="{{ route('sippar.daily-accounts.update', $expense->id, false) }}">
@csrf @method('PUT')
@include('daily_accounts._form')
<div class="da-actions"><button class="da-primary" type="submit">حفظ التعديلات</button><a class="da-btn" href="{{ route('sippar.daily-accounts.index', [], false) }}">إلغاء</a></div>
</form>
</div>
@endsection
