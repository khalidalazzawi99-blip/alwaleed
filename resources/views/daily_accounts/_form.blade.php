...
<label>الحساب المالي<select name="cashbox_id" required><option value="">اختر الحساب</option>@foreach($cashboxes as $cashbox)<option value="{{ $cashbox->id }}" @selected((string) old('cashbox_id', $expense->cashbox_id ?? '') === (string) $cashbox->id)>{{ $cashbox->name }} — {{ number_format($cashbox->balance, 2) }}</option>@endforeach</select></label>
